...
function Get-LocalSealedCaptureByKey {
    param([Parameter(Mandatory)][string]$Key)

    if ($localSealedCaptures.ContainsKey($Key)) {
        return $localSealedCaptures[$Key]
    }
    $entry = Get-LocalSealedInputByKey -Key $Key
    $capture = Read-ContainedSealedCapture -Entry $entry
    $localSealedCaptures.Add($Key, $capture)
    return $capture
}
...
function Initialize-LocalSealedInputs {
    ...
    $script:localSealedCaptures.Clear()
}
...
function Test-HistoricalSourceProjection {
    ...
    $key = [string]$Binding.source_keys[0]
    $entry = Get-LocalSealedInputByKey -Key $key
    $capture = Get-LocalSealedCaptureByKey -Key $key
...
function Test-SealedByteCaptureGuard {
    ...
    $key = 'sealed-capture-guard'
    $entry = [pscustomobject]@{
        key = $key
        relative_locator = [System.IO.Path]::GetRelativePath($repoRoot, $path)
        length = [int64]$originalBytes.LongLength
        sha256 = Get-Sha256HexFromBytes -Bytes $originalBytes
    }
    $savedInputs = $script:localSealedInputs
    $savedCaptures = $script:localSealedCaptures
    try {
        $script:localSealedInputs = @($entry)
        $script:localSealedCaptures = [System.Collections.Generic.Dictionary[string,object]]::new(
            [System.StringComparer]::Ordinal
        )
        $capture = Get-LocalSealedCaptureByKey -Key $key
        ...
        $cachedCapture = Get-LocalSealedCaptureByKey -Key $key
        $parsed = $cachedCapture.text | ConvertFrom-Json -Depth 10
        $passed = (
            [object]::ReferenceEquals($capture, $cachedCapture) -and
            $cachedCapture.stable_handle -and
            $cachedCapture.length -eq $originalBytes.LongLength -and
            $cachedCapture.sha256 -ceq (Get-Sha256HexFromBytes -Bytes $originalBytes) -and
            [string]$parsed.projection.value -ceq 'original' -and
            (Get-Sha256HexFromBytes -Bytes ([System.IO.File]::ReadAllBytes($path))) -cne
                $cachedCapture.sha256
        )
    }
    finally {
        $script:localSealedInputs = $savedInputs
        $script:localSealedCaptures = $savedCaptures
    }
...
function New-NegativeInput {
    ...
    switch ($Case.scenario) {
        ...
        'git-diff-output' {
            $inputObject.evidence.commands = @(
                (New-ScriptRecord -InputObject $inputObject -Script 'git --no-pager diff --no-textconv --no-ext-diff --output=probe.patch')
            )
        }
        ...
        'powershell-wrapper-without-no-profile' {
            Add-RequiredRead -InputObject $inputObject -Root workspace -Path 'WORK.md'
            $record = New-ReadRecord `
                -InputObject $inputObject `
                -Root workspace `
...
function Test-SealedInputs {
    ...
    foreach ($entry in @($localSealedInputs)) {
        ...
        $capture = $null
        try {
            $capture = Get-LocalSealedCaptureByKey -Key ([string]$entry.key)
            $contained = $true
        }
        catch {
            $contained = $false
...
function Test-GitReadSafetyGuards {
    ...
    $readShapes = @(
        ...
        [pscustomobject]@{ id = 'log'; unsafe = 'git log -1'; safe = 'git -c log.showSignature=false --no-pager log --no-textconv --no-ext-diff -1' },
        [pscustomobject]@{ id = 'show'; unsafe = 'git show HEAD'; safe = 'git -c log.showSignature=false --no-pager show --no-textconv --no-ext-diff HEAD' },
        [pscustomobject]@{ id = 'diff'; unsafe = 'git diff --stat'; safe = 'git --no-pager diff --no-textconv --no-ext-diff --stat' },
    ...
    foreach ($shape in @(
        [pscustomobject]@{ id = 'log'; script = 'git --no-pager log --no-textconv --no-ext-diff -1' },
        [pscustomobject]@{ id = 'show'; script = 'git --no-pager show --no-textconv --no-ext-diff HEAD' }
    )) {
    ...
    $externalDiffDisablePassed = $true
    foreach ($shape in @(
        [pscustomobject]@{ id = 'log'; script = 'git -c log.showSignature=false --no-pager log --no-textconv -1' },
        [pscustomobject]@{ id = 'show'; script = 'git -c log.showSignature=false --no-pager show --no-textconv HEAD' },
        [pscustomobject]@{ id = 'diff'; script = 'git --no-pager diff --no-textconv --stat' }
    )) {
        $externalDiffInput = New-BaseControllerInput -Candidate b965102 -CaseRoot (
            Join-Path $GuardRoot ("external-diff-disable-" + [string]$shape.id)
        )
        $externalDiffInput.evidence.commands = @(
            (New-ScriptRecord -InputObject $externalDiffInput -Script ([string]$shape.script))
        )
        $externalDiffResult = Invoke-CodexEvidenceController -Mode runtime -InputObject $externalDiffInput
        $externalDiffDisablePassed = $externalDiffDisablePassed -and (
            $externalDiffResult.adjudication.verdict -eq 'CONTROLLER_UNKNOWN' -and
            @($externalDiffResult.adjudication.unknowns) -contains 'git_diff_requires_no_ext_diff'
        )
    }
    $checks.Add([ordered]@{
        id = 'git-diff-capable-reads-require-no-ext-diff'
        passed = $externalDiffDisablePassed
    })
