...
|---|---|---|
| Changed Skill validator | Validate frontmatter and package name without changing package bytes | all three current development packages validate; Project Docs six-row manifest is `bbe5109b...`, Work Charter five-row manifest is `55fe27c1...`, and PowerShell five-row manifest is `e0cdcb25...`; immutable candidate/release identities remain separate and frozen |
| Repository checker | Validate strict UTF-8/no BOM, whitespace, local links/fragments, publication safety, exact Skill/eval sets, federated docs, bilingual navigation, promoted evaluation governance, immutable historical consumers, and the fact-first closure against the actual uncommitted target | passes at 223 text files, 14 cases, and 12 fixtures; closure `es-c1-tranche-b-current-source-convergence` exactly matches the 40-path uncommitted target, and no candidate, installed-copy, or release identity is rewritten |
...
| Codex evidence controller | Validate the existing controller plus exact current Work Charter source identity and focused convergence mutations | passes exact current-SOURCE package identity, 11/11 historical, 32/32 evidence-surface, 33/33 negative, 4/4 metamorphic, and 18/18 Work Charter convergence guards; historical candidate/release manifests remain immutable and zero external model or assessor calls occurred |
...
| PowerShell deterministic readiness and hardening | Validate exact current-package identity, static destructive-filesystem pre-error selection and near-neighbor negatives, exact `pwsh` probe readiness, WSL application-only discovery/typed exit, target-runtime parser binding, parameters, errors, text/path/cardinality, and Junction link-only removal | 59 assertions pass under exact PowerShell 7 and Windows PowerShell 5.1 executables; live WSL, model, assessor, install, elevation, and host configuration are absent |
...
| PowerShell parser sweep | Parse all `evals/**/*.ps1` files in PowerShell 7 and both portable checker entry points in Windows PowerShell 5.1 | all nine eval PowerShell scripts parse under exact PowerShell 7; `check-fixtures.ps1` and `check-powershell-boundaries.ps1` parse under exact Windows PowerShell 5.1; the PS7-only Work Charter invoker remains outside the 5.1 claim |
...
| `git diff --check` | Reject whitespace defects in the working revision | passes for the exact 40-path ES-C1 tranche B target |
| Gitleaks | Detect secret-like content independently | Gitleaks 8.30.1 scans about 2.84 MB with zero findings; ignored or protected saved-main evidence bytes are not inputs |
...
### 2026-08-19 — Current development SOURCE convergence

Tranche B keeps released identities immutable while binding three distinct
editable development identities: Project Docs six-row manifest
`bbe5109b4b9788088f7ae374f79ca1300623840282a2d88472de9e3f93822199`,
Work Charter five-row manifest
`55fe27c17b5bd023b19a59f4f9517e83db057a528dba8f9844467f11555ecf3f`,
and PowerShell five-row manifest
`e0cdcb256c38ddf4c1c0fe31f8664c7c51f66397e08a01ff5a3e9181fd9ef1bd`.
The fixture checker verifies the six immutable Project Docs `v0.1.2` Git
objects separately; the Work Charter controller leaves every historical
candidate/release manifest unchanged.
...
only causal behavior, candidate or installed-copy identity, live WSL, external
provider behavior, efficacy, installation, release, integration, or
publication.

Earlier fresh-context Work Charter and PowerShell observations remain evidence
for their exact earlier SOURCE or release revisions only; this current-source
change does not relabel or inherit them.
...
The exact five-file P1 identity at
`30668743cdc6c8be00100fd48fa89376adf746f8` was materialized into one
task-private isolated DEV copy and matched to its Git blobs and normalized
SOURCE hashes. A matched immutable `v0.1.2` control was resolved separately;
neither the installed stable copy nor persistent discovery configuration was
changed. The zero-model catalog probe surfaced exactly one intended PowerShell
entry in each isolated workspace without an omission or truncation signal.

After one corrected transport canary returned exact `TRANSPORT_OK`, four
counted fresh cells ran. The P1 positive loaded exact `SKILL.md` and its native
reference before attempting the named `.ps1`; the stable control did not load
for the same request, and ordinary-cmdlet and POSIX-only P1 negatives also did
not load. The read-only boundary declined command execution in all applicable
cells. Exact Harness executable identity remains task-private; model and
reasoning identities were not exposed and remain `UNKNOWN`. No assessor ran.
The per-Skill [Verification](skills/use-powershell-safely/VERIFICATION.md)
owns the detailed bounded conclusion and residual limits; private prompts,
logs, task identifiers, and host locators remain unpublished.
...
### 2026-08-15 — Coherent Work Charter `v0.2.0` plus PowerShell P1 local candidate

The release-prep merge has first-parent baseline
`33f0b62c4489a65959c175dfb6dfdb2b6b841689` and second-parent committed evidence
tip `a7637a680ad8ccd6f7f335a7367f46bca938b97f`; reviewed PowerShell SOURCE commit
`30668743cdc6c8be00100fd48fa89376adf746f8` is its direct ancestor. No byte was
copied from the dirty main worktree.

| Skill | Relative to immutable `v0.1.2` | Candidate package proof | Classification |
|---|---|---|---|
| Project Docs | released tree `ce6da1202e21d8968ea47c86fb9992c0dd1f6f6b` | exact same six-file tree from `ceed607...`; unreleased tree `1c42a0a...` is absent | `unchanged` |
| Work Charter | released tree `ce306f17523211a2216529e9f1ecb61502f903b0` | exact five-file tree `97f0d9de17abc9c699772e0ebdcbdaa23d890008` from `c481005...`; canonical manifest `04c382a48e43897a8806aa5ffd996984cb015d3d8cdc6e675e9181a6d94e6f44` | `changed` |
| PowerShell | released tree `22c230f720a548e603e4052b03d9f654b76d0d54` | exact five-file tree `d8bf6dc74f106c386653cb3b598d50e790c38348` from reviewed SOURCE `30668743...`; descendant `a7637a6...` adds evidence-sync facts, not SOURCE | `changed` |

All three Skill validators pass. Git tree/blob comparison proves the exact
6/5/5 path sets, strict UTF-8/no-BOM/LF/final-newline checks pass for all 16
package files, and Work Charter's path-sorted canonical JSON manifest
recomputes exactly. PowerShell readiness passes 41/41 under PowerShell 7 and
Windows PowerShell 5.1; all nine eval scripts parse under PowerShell 7 and both
changed checkers parse under Windows PowerShell 5.1. The 19-row fixture contract,
controller regression, repository/closure checker, targeted publication scan,
Gitleaks, and `git diff --check` pass. Controller output confirms zero external
model or assessor calls. Phase 1 starts zero formal, canary, product, or assessor
paths and does not rerun or reinterpret sealed D55.

Phase 2 is unstarted. Its only proposed boundary is isolated installation and
loaded-copy identity of the exact local candidate commit, one proportional Work
Charter smoke, one PowerShell positive plus two negatives, and one independent
assessor. No push, stable install/update, tag, GitHub Release, or cleanup follows
from this candidate gate.
...
### 2026-08-12 — Work Charter D50 Gate 2 terminal

D50 used a reusable controller bundle qualified through three cold shadow
materializations. The first two preserved zero-model qualification failures;
the third and exact carrier each passed eight stages / 23 checks with zero
pre-product model or assessor turns. Exact-carrier certification, bundle,
official qualification, canary-batch, freeze, and authorization hashes are
`1bd67583...`, `8b3abba4...`, `6f65c5e1...`, `9b761316...`, `a33e61ff...`, and
`f9f00575...`. All three required non-product canaries passed; freeze bound 178
files and 83 mapping rows.

D50 imported immutable D49 A01-A17 as 17 scored cells / 28 product turns.
Out-of-matrix `A18-CANDIDATE-CAL` passed in two turns. Fresh scored released-
control A18 loaded the exact control package and consumed two turns, then the
tracked controller returned `CONTROLLER_UNKNOWN` with unknown
`resolved_command_invocation_not_trusted` and no violation. D50 sealed
`SEALED_PARTIAL_HARD_STOP` at scored 17/28 with 0/11 fresh scored cells complete.
Fresh consumption was two contexts / four turns; combined consumption was 19
contexts / 32 turns and reconciled exactly. A19-A20 and both contrast groups
did not start. Terminal hash is `f49544ce...`.

The allowlisted assessor view and local disclosure review passed at
`2dbdf719...` and `43fa4b85...`. Exactly one read-only `Terra/high` assessor
returned `DECISION_REQUIRED` without an assessor hard failure; assessment hash
is `76daaac4...`. D50 is immutable, non-retriable, unrescored, and unaccepted.
The sanitized evidence is
[Work Charter D50 Gate 2 terminal](../evals/results/2026-08-12-work-charter-d50-gate2-terminal.md).
This is a controller-trust unknown, not proof of a candidate SOURCE defect or
...
attempt. The second-reset-cycle review was clean, and local commit
`410b0bd...` recorded the runner target.

These checks consumed zero model contexts, zero `turn/start`, and zero
assessor turns. They are implementation qualification, not D52 product credit.
The later fresh carrier used both private corrections and failed official stage
7 `model_authorization_envelope` before the aggregate 48/48 receipt because its
frozen run policy did not require the post-freeze authorization gate. No outer
phase receipt, canary, freeze, product, or assessor turn started. D52 is sealed
as `QUALIFICATION_FAILED / PRODUCT_UNKNOWN`; terminal receipt SHA-256 is
`7c12bbdb...abd4d`, and the candidate remains unaccepted.

### 2026-08-12 — Work Charter D53 tracked authorization lifecycle

Explicitly authorized `WC-AR-D53-LAYERED-AUTHORIZATION-LIFECYCLE` preserves the
exact candidate, released control, D51/D52 terminals, and all consumed history.
The existing runner now exposes one policy hash over the exact six-state
lifecycle, predecessor terminal receipt, and formal 56/56 threshold. Binding,
authority, carrier, and frozen-input artifacts must carry that same policy and
phase-appropriate active state. The sixteen model phases are split into three
dispatch segments so canary 3 cannot directly dispatch product and product C04
cannot directly dispatch the assessor. Real canary/product/assessor entry also
validates the exact qualification, canary-gate, freeze, and Campaign-activation
evidence prefix required for that segment; assessor entry additionally requires
the exact 28/28 admitted packet and hash-bound reviewed assessor view and
eligibility receipt. Action-provenance validation accepts only the exact D53
root and revalidates each canary's full binding/artifact/argv/successor route,
outer receipt/state, inner consumption claim, and committed runner identity.
The terminal state requires a typed bounded receipt, and acceptance additionally
requires the full product/eligibility/accepted-assessor chain. The focused
checker passes 32/32: all retained
D52 runner conditions plus exact lifecycle positive, wrong qualification,
wrong canary transition, wrong freeze, wrong activation, stale `23/23`,
predecessor-only leakage, and lifecycle hash mismatch conditions. Each negative
fails before receipt issuance and child start.

This tracked implementation consumed no model evidence. New-cycle review 3/5
found no actionable issue against its final bytes, and local commit
`4ab8a025...` records the focused 32/32 result. Fresh Layer A later preserved
four attempts and consumed all three private corrections. Final `r03` passed
materialization/finalization and qualification stages 1-6, then failed stage 7
because an inherited nested-path inference could not locate immutable D49
predecessor evidence. Layer B, aggregate 56/56, canary, freeze, product, and
assessment did not start; zero model contexts and zero `turn/start` were
consumed. D53 is sealed `QUALIFICATION_FAILED / PRODUCT_UNKNOWN` and candidate
...
The post-D53 lesson-promotion revision adds a deterministic governance and
closeout-consistency contract to the same repository checker. It requires the
project instructions, Runbook, and evaluation reference to jointly retain the
explicit external-root, zero-model pre-review feasibility, sealed-failure
review, bounded closeout-review, and all-worktree writer rules. It also binds
the immutable D53 disposition, zero-consumption boundary, deferred private-
builder repair, and no-automatic-D54 limit across the terminal report and its
existing canonical consumers. The checker is the proving consumer for those
promoted repository rules; changing a future Work Charter private builder and
its composed-path tests remains `DEFERRED` to an explicitly authorized
successor and is not claimed here.

The same revision intentionally invalidates the immutable D53 runner's current
ruleset qualification. Direct runner invocation stops before its 32 checks.
The aggregate fixture checker keeps the historical exact 32/32 branch and adds
one separately pinned drift branch requiring exit 1, no JSON result, the exact
pre-qualification error, the promoted normalized-text `AGENTS.md` identity,
and the unchanged D53 runner identity; it therefore passes 18/18 without
weakening any other nonzero or identity. Historical committed 32/32 and 18/18
evidence remains D53-only, while the current drift branch supplies no
successor, product, or acceptance credit.

### 2026-08-13 — Work Charter D54 explicit-root Build

Authorized successor `WC-AR-D54-EXPLICIT-ROOT-CONTRACT` preserves candidate
`c481005...`, manifest `04c382...`, control `ceed607...`, and every sealed
predecessor. The tracked runner/checker now accepts repository, carrier,
source-bundle, evidence, and predecessor roots only through one authenticated
outer contract, validates containment, reparse/symlink boundaries, identity or
hash, and schema, and rejects cwd/sibling/`__file__`-ancestry/directory-depth
inference. Focused coverage passes 32/32.

One disposable composed DEV route passed the real materializer, finalizer,
predecessor importer, private 24/24, aggregate 56/56, lifecycle consumer, and
synthetic next-phase dispatch. It stopped before the first `thread/start` and
recorded zero model calls, zero `thread/start`, zero `turn/start`, and zero
writes outside the declared mirror. This evidence is marked
`DEV_ONLY_NOT_FORMAL_CAMPAIGN_EVIDENCE`; it neither accepts the candidate nor
authorizes scarce evidence. Documentation/ruleset changes require one final
full-diff binding and complete DEV stability rerun before `Terra/high` native
review. Only the reviewed local commit and clean writer reconciliation may
create a Fresh Evidence Session for formal 56/56 with no correction.

D54 native review 1/5 found three P1 authentication gaps in the stable Build
target: the declared repository HEAD was only syntax-checked, source-bundle
manifest metadata was not reconciled to every actual bundle file, and
predecessor artifact-relative paths were not contained under their declared
roots. The runner now probes HEAD through the authenticated Git anchor,
recomputes the sorted unique bundle inventory and aggregate hash while rejecting
reparse or unmanifested entries, and applies containment before reading each
predecessor artifact. Focused negatives cover wrong HEAD, wrong bundle content,
and an escaping predecessor artifact path. Review evidence remains separate
from formal Gate 2 evidence.

D54 native review 2/5 found one additional P1: a seven-row ruleset array could
duplicate one valid locator while omitting another. The runner now requires the
ordinal locator multiset to equal the exact seven required inputs before
assigning or hashing the ruleset, and a duplicate-`AGENTS.md`/missing-
`evals/README.md` negative fails before lifecycle initialization.

### 2026-08-13 — Work Charter D54 formal qualification terminal

Local commit `9fe27bbd22cda34938e3e91c6c09ea72bf5a18e7` records the reviewed D54
explicit-root Build. Its clean main/index state and both registered worktrees
reconciled before the Build writer relinquished. The Fresh Evidence Session
then rebound the final ruleset, runner/checker, candidate/control, workspace/
writer, permission, and predecessor identities and created the only formal
carrier `formal-f00`.

That carrier's single formal invocation returned exit 86 at `bootstrap` with
`Index was outside the bounds of the array.` Three ignored-orchestrator pin
rows embedded `Join-Path` inside positional arrays, so PowerShell bound the
intended hash and label as extra `ChildPath` arguments; `$pin[2]` then exceeded
the single-value result. This controller/bootstrap defect occurred before
materializer, finalizer, importer, private 24/24, tracked 32/32, aggregate
56/56, lifecycle consumption, or synthetic dispatch and is not a candidate or
product finding.

No root contract or lifecycle terminal exists; the sealed summary records
`terminal=null`. Model contexts, `thread/start`, `turn/start`, and assessor
starts are 0/0/0/0, while canary/product/assessor were not authorized or
started. The immutable orchestrator, failure JSON, and failed-summary hashes
are `b0ca7a5b...a2b5`, `d406e856...9d81`, and `f94c511b...2b39`.
D54 is `QUALIFICATION_FAILED / PRODUCT_UNKNOWN`; no repair, replacement,
retry, cleanup, product assessment, or automatic D55 follows.

The repository fixture aggregate now returns exit 1 with 17/18 because the
sealed D54 outer runner rejects this closeout's intentional normalized-text
change to `evals/README.md` before running its regression cases. That expected
fail-closed result is documentation-ruleset drift after terminal disposition,
not a candidate/product finding or authority to alter the sealed runner.

### 2026-08-13 — Work Charter D55 tracked formal-bootstrap Build

Explicit successor D55 preserves D54 while moving the material bootstrap into
tracked runner mode `FormalBootstrap`. An exact ordered pin set with named
`Path`/`Sha256`/`Label` fields replaces the D54 positional-array shape. Focused
bootstrap checks pass 27/27, including the direct D54 regression, zero-write
pre-auth root rejection, independent Python executable trust negatives, typed
authenticated early-terminal pointer, pre-existing bootstrap/normal terminal
rejection, and the separately rooted revision-authority schema, containment,
commit-status, evidence-class, and runner/checker identity negatives. A hostile `GIT_DIR`/`GIT_WORK_TREE`/
`GIT_CONFIG_*` positive proves those values cannot redirect the authenticated
repository. The original tracked suite remains exactly 32/32.

The initial attempt and all four concrete corrections are retained. Corrections
1-2 authenticated the tracked subject/status set and kept bootstrap preflight
outside the fresh carrier. Correction 3 ordinal-sorted the complete subject
set. Correction 4 restored unchanged repository-checker recovery/writer
fragments and bound the DEV correction ceiling to four while formal remains at
zero corrections. A later caller-only relative-path launch failed before
bootstrap and created no stage or terminal artifact; it was not a fifth code
correction. The separately authorized one-shot R1 launcher consumed the exact
authenticated `dev-c04-final` attempt record and then passed the real
materializer, finalizer, predecessor importer, private 24/24, tracked 32/32,
aggregate 56/56, lifecycle consumer, and synthetic dispatch. It stops before
first `thread/start`; model calls/contexts, thread starts, turn starts, assessor
starts, and repository writes outside the mirror are all zero. Summary SHA-256
is `58cd551f...c266` and the evidence is explicitly DEV-only.

Native review 1/5 found two P1 trust gaps: raw failure-root use before complete
authentication and contract-supplied executable pairs that could self-authorize.
Correction 5 made pre-auth rejection typed exit 86 plus stderr with zero writes,
restricted terminal writes to authenticated `Context.FailureRoot`, and bound
Git, PowerShell, and verified Python 3.12.10 pins to independent anchors. Fresh
`dev-h1-c05` then exposed two publication-unsafe tracked absolute-path literals.
Correction 6 retained the fixed Python identity using a system-volume anchor
independent of cwd, PATH, `Get-Command`, siblings, or file ancestry. Repository
safety and focused 14/14 pass.
...
Native review 2/5 found the self-authorizing bootstrap contract/materializer
and optional formal cleanliness. Correction 7 binds materializer, manifest, and
predecessor hashes to tracked policy, permits formal only with the deterministic
D55 tool, and requires formal evidence to prove a clean checkout. Focused 16/16
adds both direct zero-write negatives. Fresh `dev-h1-c07` passes the full route;
summary SHA-256 is `7028700c...8982`, every model/start/assessor and outside-
write count is zero, H1 then stood at 7/7 corrections, and the evidence remains
DEV-only.

Native review 3/5 found two P1 environment gaps. Repository checks could honor
inherited `GIT_*` overrides despite the pinned executable, and Python stages
could honor inherited `PYTHONHOME`/`PYTHONPATH`. H2 correction 8 clears Git
overrides, supplies `safe.directory` only from the authenticated repository
root, adds Python `-I`, and removes Git/Python overrides from every Python
stage. Focused coverage then rose to 17/17. `dev-h2-c08` remained prepared-not-started
after the isolated Git prelaunch lost the sandbox safe-directory grant;
explicit rooted `safe.directory` closed that boundary. The next
`dev-h2-c08-safe` launch sealed at pre-auth exit 86 with no terminal or
model/start when the ignored H2 launcher cast every `StartsWith` Boolean to a
non-empty string and removed the full child environment. Correction 9 fixed
that launcher-only defect and a deterministic environment probe proves only
the declared Git/Python keys are removed.
...
`-I`. Summary SHA-256 is `ce4ee43a...5e5a`; start/result SHA-256 values are
`cb190a58...cea9` / `5b2de954...07ad`, and every model/context/thread/turn/
assessor/outside-write count is zero. Corrections then stood at 9/10.

Native review 4/5 found one P1 one-shot gap: an existing normal lifecycle
terminal did not block a new preflight. Correction 10 rejects either terminal
type or a failed summary after full bootstrap authentication and before any
preflight/stage write. The combined negative preserves both seeded terminal
hashes, produces exit 86, and writes no preflight/failed summary; focused
coverage is 18/18. Fresh hostile-environment `dev-h2-c10` passes private 24/24,
tracked 32/32, aggregate 56/56, lifecycle, and synthetic dispatch through
...
Native review 5/5 found one P1 because the caller-supplied bootstrap contract
still self-authorized tracked runner/checker identities. H3 correction 11 adds a
separately rooted and hash-authenticated revision-authority record and cross-
binds its exact runner/checker paths and hashes to the repository, bootstrap
pins, expected HEAD, evidence class, and reviewed-commit status before output.
The expanded suite passes 27/27. `dev-h3-c11-final` reached private 24/24 before
an outer tool transport timeout and produced no typed result; the attempt is
sealed and the timeout is not a code correction. User-authorized one-shot R2
created fresh `dev-h3-c11-r2` and completed bootstrap, materializer, finalizer,
predecessor importer, private 24/24, tracked 32/32, aggregate 56/56, lifecycle,
and synthetic dispatch through `STOPPED_BEFORE_FIRST_THREAD_START`. Its 960-
second outer call used 60-second waits and completed in 464.6 seconds. Summary,
start, and result SHA-256 values are `cb883af9...5385`, `2e3da026...3f16`, and
...
corrections 13-15 remain unused. This is DEV implementation qualification only;
the new native-review cycle is next.
...
Native review 2/5 found that the formal v2 policy was not tied to the exact
reviewed repository revision. Correction 14 adds the narrow tracked formal-
authority owner; the runner requires its fixed hash and independently anchored
Git last-change commit to equal contract and actual authenticated HEAD. The
wrong-HEAD negative raises focused coverage to 29/29. Fresh
`dev-h3-c14-final` passes 56/56/lifecycle/synthetic dispatch through
`STOPPED_BEFORE_FIRST_THREAD_START`, with every model/context/thread/turn/
assessor/outside-write count zero. Correction 14 is consumed, correction 15 is
unused, and review 3/5 is next.

Review 3/5 repeated the repository-local self-authorization P1. User-approved
`CONTROL_PLANE_BOUND A′` correction 15 deletes the untracked authority proposal
and removes the v2 hash anchor, anchor-zeroed hash, tracked-owner last-change,
review-status, and review-count checks. The retained v1 record proves only
mechanical evidence-class, exact-HEAD/branch, schema, containment, and raw
runner/checker path/hash integrity against the bootstrap pins. A review-status
field is rejected as extra. Review approval and selection of the exact commit
eligible for Fresh Evidence remain user/Planner/Harness control-plane actions.
Correction 16 is reserved; fresh focused and composed DEV checks precede the
new at-most-three-review cycle.

D54's three sealed SHA-256 values remain `b0ca7a5b...a2b5`,
`d406e856...9d81`, and `f94c511b...2b39`. The D54 lesson becomes `PROMOTED`
only in the reviewed local D55 Build commit containing the tracked runner/checker and
proving focused plus complete composed DEV checks. This Build starts no D55 formal, canary, product, or assessor
evidence and creates no D56 authority.
...
### 2026-08-14 — Work Charter minimal claim-level evaluation

Exact candidate `c4810057c3f28cca9f12004ca2018784cd21f449` was mechanically
materialized as five files matching manifest
`04c382a48e43897a8806aa5ffd996984cb015d3d8cdc6e675e9181a6d94e6f44`.
The zero-model route passed with three isolated roots, zero corrections,
canaries, and product starts. Claim synthesis reused only admitted D45-D49
exact-candidate evidence; one independent read-only assessor agreed that C1-C5
are each `ACCEPT`. The bounded overall verdict is therefore `ACCEPT` for those
five claims only. D44/D47/D50 and D51-D55 remain excluded product evidence;
complete 28-cell Gate 2 and release readiness remain unaccepted.

D55 later sealed C17 as `QUALIFICATION_FAILED / PRODUCT_UNKNOWN`; exact failure
mechanism remains `UNKNOWN`, no formal/product/assessor evidence was created,
and its writer relinquished. The minimal evaluation did not repair, import, or
replay D55.

The sanitized terminal is
...
### 2026-08-14 — Work Charter-only `v0.2.0` prerelease deterministic gate

The task-owned release-prep checkout proves the public package set as Work
Charter 5/5 Git blobs equal to exact `c481005...`, Project Docs 6/6 and
PowerShell 5/5 Git blobs equal to exact `ceed607...`, with Work Charter
canonical manifest `04c382a4...`. All 12 allowlisted factual-source raw hashes
match the authenticated source root.

All three Skill validators pass. PowerShell 7 and Windows PowerShell 5.1 each
pass 15/15 boundary checks; all ten tracked PowerShell scripts/module files
parse. The controller returns `PASS` with no failures, and the revision-aware
fixture checker passes 18/18. Its two release-set branches bind exact `v0.1.2`
Project Docs bytes before classifying the frozen Decision 0019 fixture as
excluded, and bind the exact unchanged release-prep runner before accepting
the absence of its declared ignored D50 private carrier as a pre-qualification
stop. Neither branch creates product or acceptance evidence.

The repository checker passes at 218 text files / 14 cases / 12 fixtures.
Targeted Gitleaks scans all 27 intended public paths without reading ignored
