...
|---|---|---|
| Changed Skill validator | Validate frontmatter and package name without changing the package | unchanged exact `c481005...` Work Charter package passes the bundled `skill-creator` validator; D53 changes no Skill SOURCE |
| Repository checker | Validate strict UTF-8/no BOM, whitespace, local links/fragments, publication safety, exact Skill/eval sets, federated docs, bilingual navigation, the promoted project evaluation-governance contract, and immutable D53 closeout-consumer consistency | passes for the post-D53 governance target with 214 text files / 14 cases / 12 fixtures; the check changes no Skill SOURCE or sealed evidence |
| Fixture checker | Validate synthetic starting states and their Git/reparse/ignored-result boundaries plus tracked controller and runner regressions | passes 18/18 for the post-D53 governance target. Seventeen ordinary rows pass; the runner row admits either the historical exact 32/32 result or, for this revision, only exit 1 plus no JSON result, the exact pre-qualification drift message, exact promoted normalized-text `AGENTS.md` identity, and unchanged D53 runner identity. Every other nonzero or identity fails |
| Work Charter Gate 2 outer runner | Validate one tracked consuming entry, exact Campaign/candidate identity, authenticated explicit repository/carrier/source-bundle/evidence/predecessor roots, containment/reparse/hash/schema boundaries, the retained authorization lifecycle, formal aggregate through canary/product/assessor dispatch, typed exits, one-shot receipts, and stop-before-start qualification | explicitly authorized D54 focused checks pass 32/32, including nested-positive, decoy-sibling, wrong/outside/reparse/missing/hash/schema negatives. A composed disposable DEV run also passes private 24/24 and aggregate 56/56, then stops synthetic dispatch before first `thread/start` with zero model/start events. These are DEV-only implementation results; formal qualification awaits the reviewed committed D54 revision and Fresh Evidence Session |
...
| Work Charter D36/D37/D40, D34/D41 predecessors, and D42-D54 states | Preserve additive predecessors and separate product behavior from route, tool, qualification, eval-contract, authorization, environment, private-carrier failure, and DEV-only successor implementation | D36-D53 remain immutable with their recorded dispositions; D53 stays `QUALIFICATION_FAILED / PRODUCT_UNKNOWN` and its terminal receipt is unchanged. D54 passes only disposable explicit-root implementation qualification with zero model turns; it supplies no formal or product credit and exact candidate `c481005...` remains unaccepted |
...
| `git diff --check` | Reject whitespace defects in the working revision | passes for the exact eleven-path D53 terminal closeout target; index remains untouched |
| Gitleaks | Detect secret-like content independently | exact per-file scans of the eleven-path D53 terminal closeout target pass with zero findings; no ignored-carrier content was scanned or is eligible for publication |
