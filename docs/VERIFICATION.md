...
|---|---|---|
| Changed Skill validator | Validate frontmatter and package name without changing the package | unreleased P1 `use-powershell-safely` package passes the bundled `skill-creator` validator; released and installed copies are unchanged |
| Repository checker | Validate strict UTF-8/no BOM, whitespace, local links/fragments, publication safety, exact Skill/eval sets, federated docs, bilingual navigation, the promoted project evaluation-governance contract, immutable D53 closeout-consumer consistency, and an optional fact-first documentation-impact closure against the actual uncommitted Git target | current P1 target passes with 216 text files / 14 cases / 12 fixtures; closure self-tests reject an omitted bounded consumer and accept the complete mapping; no sealed evidence changes |
| Fixture checker | Validate synthetic starting states and their Git/reparse/ignored-result boundaries plus tracked controller and runner regressions | the P1 target passes all four PowerShell rows and 18/19 aggregate rows. The remaining excluded Work Charter Gate 2 runner regression was already present in the clean base and remains nonzero; no Work Charter change, retry, or PowerShell defect attribution follows |
...
| Work Charter D36/D37/D40, D34/D41 predecessors, and D42-D54 states | Preserve additive predecessors and separate product behavior from route, tool, qualification, eval-contract, authorization, environment, private-carrier failure, and DEV-only successor implementation | D36-D53 remain immutable with their recorded dispositions; D53 stays `QUALIFICATION_FAILED / PRODUCT_UNKNOWN` and its terminal receipt is unchanged. D54's DEV explicit-root pass supplies no formal credit; its only formal carrier is sealed at bootstrap as `QUALIFICATION_FAILED / PRODUCT_UNKNOWN`, with no product evidence and candidate `c481005...` still unaccepted |
...
| `git diff --check` | Reject whitespace defects in the working revision | passes for the exact eight-path PowerShell P1 selection-evidence sync target; index remains untouched |
| Gitleaks | Detect secret-like content independently | exact uncommitted-diff scan of the eight publication paths covers about 25 KB and passes with zero findings; ignored private selection evidence remains outside publication scope |
