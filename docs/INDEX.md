...
| [`docs/decisions/0018-work-charter-adoption-levels-and-reentry-checkpoint.md`](decisions/0018-work-charter-adoption-levels-and-reentry-checkpoint.md) | Work Charter native-package/explicit-activation entry, proportional protection levels, durable re-entry, task convergence, evaluation, workspace, and lifecycle gates | accepted through `WC-AR-D49-EVAL-CONTRACT`; D47 is an immutable pre-product provider-availability stop, D48 is immutable and unaccepted at combined 13/27 after A14 lacked frozen loaded-copy proof under an ambiguous read-scope prompt, and the prospective 20-run/28-cell eval contract now separates project/evidence read scope from package loading plus direct package-denial/Harness-preload attribution; no D49 Campaign is authorized |
